<form method="POST" action="{{ route('admin.tabs.store') }}" class="space-y-6">
    ...
    <div class="flex items-center justify-end mt-6 pt-4 border-t border-gray-200">
        <button type="button" onclick="if (typeof closeTabCreateModal === 'function') { closeTabCreateModal(); } else { window.location.href = '{{ route('admin.cards.index') }}'; }" class="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-400 focus:bg-gray-400 active:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150 mr-2">
            Cancelar
        </button>
