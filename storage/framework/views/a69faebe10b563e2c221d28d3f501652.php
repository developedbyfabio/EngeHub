...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.page-header','data' => ['title' => 'Câmeras','icon' => 'fas fa-video']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Câmeras','icon' => 'fas fa-video']); ?>
         <?php $__env->slot('actions', null, []); ?> 
            ...
            <?php if(auth()->user()?->hasFullAccess() ?? false): ?>
                <a href="<?php echo e(route('admin.cameras.index')); ?>" class="page-header-btn-secondary">
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <?php if($checklistsEmAndamento->count() > 0): ?>
                    <ul id="checklists-em-andamento" class="space-y-3">
                        <?php $__currentLoopData = $checklistsEmAndamento; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="flex items-center justify-between gap-4 p-3 rounded-lg hover:bg-gray-50">
                                <a href="<?php echo e(route('cameras.checklists.show', $c)); ?>" class="inline-flex items-center text-blue-600 hover:underline font-medium flex-1">
                                    <?php $totalDvrs = $dvrs->count(); $mostrarTodos = $totalDvrs > 0 && $c->dvrs->count() >= $totalDvrs; $dvrNomes = $mostrarTodos ? 'Todos os DVRs' : ($c->dvrs->count() > 0 ? $c->dvrs->pluck('nome')->join(', ') : ($c->dvr?->nome ?? '-')); ?>
                                    <?php echo e($dvrNomes); ?> — iniciado <?php echo e($c->iniciado_em->format('d/m/Y H:i')); ?>

                                    <i class="fas fa-arrow-right ml-2 text-sm"></i>
                                </a>
                                <button type="button" onclick="abandonarChecklist(this)" data-cancel-url="<?php echo e(route('cameras.checklists.cancelar', $c)); ?>" data-dvr-nome="<?php echo e(e($dvrNomes)); ?>" class="text-red-600 hover:text-red-800 text-sm font-medium" title="Abandonar checklist">
        ...
        <div id="iniciarChecklistModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center">
            <div class="w-11/12 md:w-2/3 max-w-2xl shadow-lg rounded-md bg-white m-4">
                ...
                <div class="px-6 pb-6">
                    <form action="<?php echo e(route('cameras.checklists.store')); ?>" method="POST" id="formIniciarChecklist" class="space-y-4">
                        <?php echo csrf_field(); ?>
                        <div>
                            ...
                            <div class="space-y-1 max-h-80 overflow-y-auto border border-gray-200 rounded-md p-3 bg-gray-50">
                                ...
                                <?php $__currentLoopData = $dvrs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dvr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="dvr-block" data-dvr-id="<?php echo e($dvr->id); ?>">
                                    <div class="flex items-center gap-1 p-2 rounded -m-1 hover:bg-gray-100">
                                        <button type="button" class="expand-dvr-btn text-gray-500 hover:text-gray-700 p-0.5" data-dvr-id="<?php echo e($dvr->id); ?>" title="Expandir para selecionar câmeras">
                                        ...
                                        <label class="flex-1 cursor-pointer flex items-center">
                                            <input type="checkbox" name="dvr_ids[]" value="<?php echo e($dvr->id); ?>" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500 dvr-checkbox">
                                            <span class="ml-2 text-gray-800"><?php echo e($dvr->nome); ?> (<?php echo e($dvr->cameras_count ?? $dvr->cameras->count()); ?> câmeras)</span>
                                        </label>
                                    </div>
                                    <div class="dvr-cameras-list hidden pl-6 pr-2 pb-2 space-y-1 ml-6 border-l-2 border-gray-200 mt-1" data-dvr-id="<?php echo e($dvr->id); ?>">
                                        <?php $__currentLoopData = $dvr->cameras ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cam): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-center cursor-pointer hover:bg-gray-100 p-2 rounded -m-1 camera-checkbox-label" data-dvr-id="<?php echo e($dvr->id); ?>">
                                            <input type="checkbox" name="camera_ids[]" value="<?php echo e($cam->id); ?>" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500 camera-checkbox" data-dvr-id="<?php echo e($dvr->id); ?>">
                                            <span class="ml-2 text-sm <?php echo e($cam->status === 'aguardando_correcao' ? 'text-amber-700 font-medium' : 'text-gray-700'); ?>"><?php echo e($cam->nome); ?></span>
                                            <?php if($cam->status === 'aguardando_correcao'): ?>
                        ...
                        <div>
                            ...
                            <input type="text" id="modal_responsavel_nome" name="responsavel_nome" value="<?php echo e(auth()->user()?->name ?? auth()->guard('system')->user()?->name ?? 'Operador'); ?>"
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <form method="GET" action="<?php echo e(route('cameras.index')); ?>" class="flex flex-wrap gap-4 mb-6">
                    <div class="flex items-center gap-2">
                        <label class="text-sm font-medium text-gray-700">Período:</label>
                        <select name="period" class="border-gray-300 rounded-md shadow-sm text-sm" onchange="this.form.submit()">
                            <option value="today" <?php echo e(($period ?? 'week') == 'today' ? 'selected' : ''); ?>>Hoje</option>
                            <option value="week" <?php echo e(($period ?? 'week') == 'week' ? 'selected' : ''); ?>>Últimos 7 dias</option>
                            <option value="month" <?php echo e(($period ?? '') == 'month' ? 'selected' : ''); ?>>Últimos 30 dias</option>
                        </select>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <select name="dvr_id" class="border-gray-300 rounded-md shadow-sm text-sm" onchange="this.form.submit()">
                            ...
                            <?php $__currentLoopData = $dvrs ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($d->id); ?>" <?php echo e(($dvrFilter ?? '') == $d->id ? 'selected' : ''); ?>><?php echo e($d->nome); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                    </div>
                </form>

                <?php if(isset($checklistsFinalizados) && $checklistsFinalizados->count() > 0): ?>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php $__currentLoopData = $checklistsFinalizados; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 text-sm font-medium text-gray-900"><?php echo e($dvrNomesHist); ?></td>
                                        <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($c->iniciado_em->format('d/m/Y H:i')); ?></td>
                                        <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($c->finalizado_em?->format('d/m/Y H:i') ?? '-'); ?></td>
                                        <td class="px-4 py-3 text-sm text-gray-600"><?php echo e($c->responsavel); ?></td>
                                        <td class="px-4 py-3 text-sm"><?php echo e($c->itens->count()); ?></td>
                                        <td class="px-4 py-3 text-sm"><span class="text-green-600 font-medium"><?php echo e($online); ?></span></td>
                                        <td class="px-4 py-3 text-sm"><span class="text-red-600 font-medium"><?php echo e($problema); ?></span></td>
                                        <td class="px-4 py-3 text-sm text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="<?php echo e(route('cameras.checklists.detalhes', $c)); ?>" class="p-2 text-blue-600 hover:bg-blue-50 rounded" title="Detalhes">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <?php if($c->finalizado_em): ?>
                                                    <a href="<?php echo e(route('cameras.checklists.pdf', $c)); ?>" target="_blank" class="p-2 text-red-600 hover:bg-red-50 rounded" title="Visualizar PDF">
                                                        <i class="fas fa-file-pdf"></i>
                                                    </a>
                                                    <a href="<?php echo e(route('cameras.checklists.pdf.download', $c)); ?>" class="p-2 text-gray-600 hover:bg-gray-100 rounded" title="Baixar PDF">
...
function confirmarApagarHistorico() {
    ...
    fetch('<?php echo e(route("cameras.historico.apagar")); ?>', {
        method: 'POST',
        body: fd,
        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
    })
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /srv/www/Engehub/resources/views/cameras/index.blade.php ENDPATH**/ ?>