...
<head>
    ...
    <title><?php echo e(config('app.name', 'EngeHub - Intranet')); ?></title>
    
    <!-- Favicon -->
    <link rel="icon" type="image/png" href="<?php echo e(asset('media/favicon.png')); ?>">
    <link rel="shortcut icon" type="image/png" href="<?php echo e(asset('media/favicon.png')); ?>">
    <link rel="apple-touch-icon" href="<?php echo e(asset('media/favicon.png')); ?>">
...
<body class="font-sans antialiased custom-wallpaper-bg">
    ...
    <?php echo $__env->make('components.toast-notification', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
    ...
    ?>
    <div id="loginModal" 
         class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[9999] <?php echo e($requireLogin ? 'show' : 'hidden'); ?>" 
         style="<?php echo e($requireLogin ? 'display: flex;' : 'display: none;'); ?>" 
         data-required="<?php echo e($requireLogin ? 'true' : 'false'); ?>"
        ...
        <div id="loginModalContent" class="modal-dark rounded-2xl p-6 max-w-md w-full mx-4 text-center" style="border-radius: 24px !important; overflow: hidden;" onclick="event.stopPropagation()">
            ...
            <!-- Formulário de Login -->
            <form id="modalLoginForm" method="POST" action="<?php echo e(route('login')); ?>">
                <?php echo csrf_field(); ?>
                ...
                <!-- Botões -->
                <div class="flex <?php echo e($requireLogin ? 'justify-center' : 'space-x-3'); ?>">
                    <?php if(!$requireLogin): ?>
                    ...
                    <?php endif; ?>
                    <button type="submit" 
                            class="<?php echo e($requireLogin ? 'w-full' : 'flex-1'); ?> px-4 py-2 bg-blue-600 border border-transparent rounded-xl text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#1f1f1f] focus:ring-blue-500 transition-colors duration-150"
                            style="border-radius: 12px !important;">
    ...
    <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                setTimeout(function() {
                    if (typeof window.showToast === 'function') {
                        window.showToast('<?php echo e(addslashes(session('success'))); ?>', 'success', 5000);
                    } else {
                        createFallbackToast('<?php echo e(addslashes(session('success'))); ?>');
                    }
                }, 200);
            });
        <?php endif; ?>
        <?php if(session('error')): ?>
            document.addEventListener('DOMContentLoaded', function() {
                setTimeout(function() {
                    if (typeof window.showToast === 'function') {
                        window.showToast('<?php echo e(addslashes(session('error'))); ?>', 'error', 5000);
                    }
                }, 200);
            });
        <?php endif; ?>
...
</html> <?php /**PATH /srv/www/Engehub/resources/views/layouts/app.blade.php ENDPATH**/ ?>